...
class CartController extends Controller
{
    ...
    public function create_order(Request $rq){
        ...
        $datacate = array();
          for($i=0; $i < count($product_name); $i++){
                // creat table join
                // $datacate=array('product_name'=>$product_name[$i]);
                // $product = DB::table('ordered_product')
                // ->leftJoin('products', 'ordered_product.product_id', '=', 'products.id')
                // ->select('ordered_product.product_name','products.category_id')
                // ->where('ordered_product.product_name',$datacate)
                // ->insert(['products.category_id'=>DB::table('')]);
                 // creat table join
